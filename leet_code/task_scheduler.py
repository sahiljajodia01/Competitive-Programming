...
sorted_d = OrderedDict(sorted(dic.items(), key=lambda x: x[1], reverse=True))
sol = []
print(sorted_d)
...
while True:
    ...
    for key, val in sorted_d.items():
        if val > 0 and key == list(sorted_d.keys())[0]:
            sol.append(key)
            sorted_d[key] -= 1
        else:
            if k > 0:
                if count < k and val > 0:
                    sol.append(key)
                    sorted_d[key] -= 1
                    count += 1
            else:
    ...
    sorted_d = OrderedDict(sorted(sorted_d.items(), key=lambda x: x[1], reverse=True))
